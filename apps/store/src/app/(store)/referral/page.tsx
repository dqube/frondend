...
const REFERRAL_HISTORY = [
  { name: "Sarah M.", status: "completed", reward: "RM 5.00", date: "Jan 5, 2025" },
  { name: "Alex K.", status: "pending", reward: "RM 5.00", date: "Jan 10, 2025" },
  { name: "Emily R.", status: "completed", reward: "RM 5.00", date: "Dec 20, 2024" },
];
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      >
        ...
        <h2 className="text-lg font-bold">Give RM 10, Get RM 5</h2>
        <p className="text-sm text-muted-foreground max-w-md mx-auto">
          Share your referral code with friends. They get RM 10 off their first order, and you earn RM 5 in store credit!
        </p>

        {/* Code */}
        <div className="flex items-center justify-center gap-2 mt-4">
          <span className="font-mono text-xl font-bold bg-background/80 px-6 py-2.5 rounded-xl border border-border/50">{REFERRAL_CODE}</span>
          <button
            onClick={copyCode}
        ...
        <div className="flex justify-center gap-2 pt-2">
          {["WhatsApp", "Email", "SMS", "Link"].map((channel) => (
            <button
              key={channel}
              className="px-4 py-2 rounded-xl bg-background/80 border border-border/50 text-xs font-medium hover:bg-muted/60 transition-colors flex items-center gap-1.5"
            >
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          { label: "Credits Earned", value: "RM 15" },
          { label: "Pending", value: "RM 5" },
        ].map((stat, i) => (
          <motion.div
            key={stat.label}
        ...
        <div className="bg-card/60 backdrop-blur-sm border border-border/50 rounded-2xl shadow-sm p-5">
          <div className="grid gap-4 sm:grid-cols-3">
            {[
              ...
              { step: "2", title: "They Shop", desc: "Your friend gets RM 10 off their first order" },
              { step: "3", title: "You Earn", desc: "Get RM 5 credit when they complete an order" },
            ].map((s) => (
