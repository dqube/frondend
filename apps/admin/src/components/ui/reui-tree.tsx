...
function TreeItem<T = any>({
  ...
  return (
    <TreeContext.Provider value={{ ...parentContext, currentItem: item }}>
      <Comp {...(defaultProps as any)} {...(otherProps as any)}>
        {children}
      </Comp>
    </TreeContext.Provider>
  );
...
function TreeItemLabel<T = any>({
  ...
  return (
    <Comp
      ...
      className={cn(
        "in-focus-visible:ring-ring/50 bg-background hover:bg-accent in-data-[selected=true]:bg-accent in-data-[selected=true]:text-accent-foreground in-data-[drag-target=true]:bg-accent flex items-center gap-1 rounded-sm transition-colors not-in-data-[folder=true]:ps-7 in-focus-visible:ring-[3px]",
        "py-1.5 px-2 text-sm",
        className
      )}
    ...
    >
      <Fragment>
        {item.isFolder() &&
          (toggleIconType === "plus-minus" ? (
            item.isExpanded() ? (
              <Minus className="text-muted-foreground size-3.5" strokeWidth={1} />
            ) : (
              <Plus className="text-muted-foreground size-3.5" strokeWidth={1} />
            )
          ) : (
            <ChevronDown
              className={cn(
